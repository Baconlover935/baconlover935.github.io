...
NICK.prime.Player = function (_config) {
	...
	this.defaults = {
		...
		'templateURL': 'https://web.archive.org/web/20141009030303/http://media.mtvnservices.com/{uri}'
	};
...
NICK.prime.Player.prototype.playerEvent = function () {
	...
	*/
	return function (event) {
		...
		//These should not be depended on, please update your code to the new API http://mtvn-player.github.io/embed-api/docs/#!/api/MTVNPlayer.Events
		switch(event.type){
			case 'playlistComplete':
				controller.trigger('onPlaylistComplete', event);
				//console.log('########EVENT TYPE OLD########## onPlaylistComplete');
				break;
			case 'mediaStart':
				controller.trigger('onMediaStart', event);
				//console.log('########EVENT TYPE OLD########## onMediaStart');
				break;
			case 'mediaEnd':
				controller.trigger('onMediaEnd', event);
				//console.log('########EVENT TYPE OLD########## onMediaEnd');
				break;
			case 'ready':
				controller.trigger('onPlayerReady', event);
				//console.log('########EVENT TYPE OLD########## onPlayerReady');
				break;
			case 'stateChange':
				controller.trigger('onStateChange', event);
				//console.log('########EVENT TYPE OLD########## onStateChange');
				break;
